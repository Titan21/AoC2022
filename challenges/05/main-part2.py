...
def move_9001(layout, amount, pos_from, pos_to):

    moving = []
    for _ in range(amount):
        moving.append(layout[pos_from - 1].pop())

    for crate in reversed(moving):
        layout[pos_to - 1].append(crate)
    return layout
...
def perform_operations(layout: List[str], instructions: List[Instuction]):
    ...
    for instruction in instructions:
        ...
        layout = move_9001(layout, instruction.amount, instruction.pos_from, instruction.pos_to)
